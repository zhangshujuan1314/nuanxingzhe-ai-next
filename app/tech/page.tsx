...
export default function TechPage() {
  const layers = [
    { y: 20, label: '前端展示层', color: '#D97706', items: ['响应式Web', 'iOS', 'Android', '小程序', '桌面端', '地图插件'] },
    { y: 105, label: 'AI 对话层', color: '#2563EB', items: ['意图识别', '场景路由', '大模型对话生成', '安全风控'] },
    { y: 190, label: '数据服务层', color: '#059669', items: ['楼栋/电梯实时数据', '订单数据', '信用积分引擎', '微任务调度', '治安事件流'] },
    { y: 275, label: '硬件接入层', color: '#8E8E8E', items: ['电梯物联网', '门禁系统', '社区摄像头(脱敏)', '骑手定位'] },
  ];
  ...
  const rects = layers.map(l => {
    ...
    return `<rect x="0" y="${l.y}" width="800" height="70" rx="8" fill="${l.color}08" stroke="${l.color}30" stroke-width="1"/>
      ...
      <text x="${startX + i * (itemW + gap) + itemW / 2}" y="${l.y + 50}" text-anchor="middle" fill="#5C5C5C" font-size="10">${item}</text>`).join('')}`;
  }).join('');

  const arrows = [1, 2, 3].map(i =>
    `<line x1="400" y1="${layers[i - 1].y + 70}" x2="400" y2="${layers[i].y}" stroke="#E8E5E1" stroke-width="1.5" marker-end="url(#arrow)"/>`
  ).join('');
    ...
    <div className="max-w-5xl mx-auto px-6 pt-16 pb-12">
      ...
      <div className="glass p-6 mb-8">
        <svg viewBox="0 0 800 340" className="w-full" dangerouslySetInnerHTML={{
          __html: `<defs><marker id="arrow" viewBox="0 0 10 10" refX="5" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse"><path d="M 0 0 L 10 5 L 0 10 z" fill="#E8E5E1"/></marker></defs>${rects}${arrows}<text x="400" y="85" text-anchor="middle" fill="#E8E5E1" font-size="10">↓ 由 LLM 驱动的自然语言交互</text>`
        }} />
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div className="card">
          ...
          <ul className="text-sm text-[#5C5C5C] space-y-1.5">
            <li><span className="text-[#141414]">前端：</span>响应式 Web / iOS / Android / 小程序 / 桌面端 / 地图插件</li>
            <li><span className="text-[#141414]">对话与风控：</span>LLM 驱动自然语言交互 + 意图识别 + 场景路由 + 安全风控</li>
            <li><span className="text-[#141414]">数据与物联：</span>实时电梯状态、门禁系统、社区摄像头（脱敏）、骑手定位</li>
          </ul>
        </div>
        <div className="card">
        ...
        <div className="card">
          ...
          <p className="text-[#8E8E8E] text-xs mt-3">以上为概念示意，不实现后端。</p>
        </div>
      </div>
    </div>
  );
